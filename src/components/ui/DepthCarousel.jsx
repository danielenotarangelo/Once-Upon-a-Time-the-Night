...
export default function DepthCarousel({
  ...
  const layout = useCallback(pos => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Symmetric depth stack: previous cards recede to one side, next cards to the other —
      // both stay visible around the active (d === 0) card, unlike a one-directional fan.
      const az = Math.abs(d);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      const cfg = cfgRef.current;
      // Only the active card's width drives the fit — side peeks are allowed to run past the
      // container edge (naturally cropped by the viewport) rather than shrinking everything down.
      const needed = cfg.cardWidth + 24;
